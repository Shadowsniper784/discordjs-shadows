module.exports = {
  ...
  callback: async ({ message, args, instance }) => {
    ...
    const { guild } = channel;
    const { ShadowCommand } = instance;
    const { MessageEmbed } = require('discord.js')
    const global = await ShadowCommand.slashCommands();
    ...
    if (args.length && args[0] === "delete") {
      ...
      if (useGuild) {
        ...
        channel.send(
          `Slash command with the ID "${targetCommand}" has been deleted. This may take up to 1 hour to be seen on all servers using this bot..`
        );
      }
      return;
    }
    ...
    const embed = new MessageEmbed()
      .addField(
        ...
        `_${instance.ShadowCommand.getPrefix(guild)}slash delete <command ID>`
      )
      .addField(
    ...
    if (1===2) {
      const guildOnly = await instance.slashCommands.get(guild.id);
